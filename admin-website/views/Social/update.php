<?php 
      ...
      if(isset($_POST['btn-update'])) {
        ...
        if(isset($_FILES['file']) && $_FILES['file']['error'] == 0) {
          $Social->attach($_FILES['file']);   
        }
        if($Social->prepare($_POST)->save()) {
          $session->set_message("data updated successfully.");
        } else {
          $session->set_message($Social->errors);
        }
      }
      ...
      <?php 
        if(isset($_GET['id'])) {
          $record = $Social->prepare($_GET)->fetch();
          if(!$record) {
                      ...
                      <form method="post" enctype="multipart/form-data">
                        <div class="col-md-6 left-part">
                              ...
                              <div class="form-group">
                                ...
                                <input class="form-control" placeholder="Title" name="title" type="text" value="<?php if(isset($record)){echo $record->title;} ?>">
                              </div>        
                              <div class="form-group">
                                <input type="hidden" value="<?php if(isset($record)) {echo $record->id;} ?>" name="id">
                                <input class="form-control" placeholder="Links" name="link" type="text" value="<?php if(isset($record)){echo $record->link;} ?>">
                              </div>
                              <button type="submit" class="btn btn-primary" name="btn-update">
                                <span class="glyphicon glyphicon-edit"></span> Update Record
                              </button>
                              <a href="index.php?page=Social_read" class="btn btn-success"><span class="glyphicon glyphicon-backward"></span>&nbsp; Cancel</a>                  
