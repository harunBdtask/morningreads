...
class User {
	...
	private $extensions = array("jpg","png","bmp");
	public $id;
	public $username;
	public $password;
	...
	public function authenticate() {
		try {
			$stmt = $this->db->prepare("SELECT * FROM ".self::$table." where username = :usename and password=:password");
			$md5 = md5($this->password);
			//$sha1 = sha1($md5);
			//$salt = "$2s*@88fjksdlfj%^&";
			//$crypt = crypt($sha1,$salt);
			$stmt->execute(array(":usename" => $this->username,":password" => $md5));
			if($stmt->rowCount() > 0) {
				return $stmt->fetch(5);
			}
		} catch (PDOException $e) {
	...
	public function save() {
		...
		foreach ($this->db_fields() as $field) {
			...
			if($field == "Mission_title" || $field == "Mission_description") {
				if(empty($this->$field)) {					
