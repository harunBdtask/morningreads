...
class Slide {
	private $db;
	private static $table="slideshow";
	...
	public function fetch_visitCount() {
		try {
			$stmt = $this->db->prepare("SELECT id, SUBSTRING(`title`, 1, 50) AS title, file, SUBSTRING(`des`, 1, 100) AS des, create_at, visits FROM `slideshow` ORDER BY `slideshow`.`visits`  DESC");
			//$stmt = $this->db->prepare("SELECT * FROM ".self::$table);
			$stmt->execute();
			$records = array();
			if($stmt->rowCount() > 0) {
	...
	public function fetch_substring() {
		try {
			$stmt = $this->db->prepare("SELECT id, SUBSTRING(`title`, 1, 20) AS title, file, SUBSTRING(`des`, 1, 50) AS des FROM ".self::$table." ORDER BY `slideshow`.`create_at` DESC");
			//$stmt = $this->db->prepare("SELECT * FROM ".self::$table);
			$stmt->execute();
			$records = array();
			if($stmt->rowCount() > 0) {
	...
	public function fetch_substringLimit() {
		try {
			$stmt = $this->db->prepare("SELECT id, SUBSTRING(`title`, 1, 50) AS title, file, SUBSTRING(`des`, 1, 100) AS des FROM ".self::$table." ORDER BY `slideshow`.`create_at` DESC LIMIT 1");
			//$stmt = $this->db->prepare("SELECT * FROM ".self::$table);
			$stmt->execute();
			$records = array();
			if($stmt->rowCount() > 0) {
